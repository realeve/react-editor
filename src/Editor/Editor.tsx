...
function undoSelectTargets({ prevs, nexts }: IObject<any>, editor: Editor) {
    editor.setSelectedTargets(
        editor.viewport.current!.getElements(prevs),
        true
    );
}
function redoSelectTargets({ prevs, nexts }: IObject<any>, editor: Editor) {
    editor.setSelectedTargets(
        editor.viewport.current!.getElements(nexts),
        true
    );
}
...
const rangeX = [-range.padding, range.width + range.padding],
    rangeY = [-range.padding, range.height + range.padding];
...
> {
        ...
        return (
            ...
            >
                ...
                <InfiniteViewer
                    ...
                    onScroll={(e: OnScroll) => {
                        if (
                            e.scrollTop < rangeY[0] ||
                            e.scrollLeft < rangeX[0] ||
                            e.scrollLeft > rangeX[1] - 1300 ||
                            e.scrollTop > rangeY[1] - 800
                        ) {
                            return;
                        }

                        horizontalGuides.current!.scroll(e.scrollLeft);
                        horizontalGuides.current!.scrollGuides(
                            Math.max(e.scrollTop, rangeY[0])
                        );

                        verticalGuides.current!.scroll(
                            Math.max(e.scrollTop, rangeY[0])
                        );
                ...
                <Selecto
                    ...
                    hitRate={0}
                    selectableTargets={[
                        `.scena-viewport [${DATA_SCENA_ELEMENT_ID}]`,
                    ]}
                    ...
                    onDrag={(e: { deltaX: number; deltaY: number }) => {
                        ...
                        infiniteViewer.current!.scrollBy(-e.deltaX, -e.deltaY);
                    }}
    ...
    public appendComplete(infos: ElementInfo[], isRestore?: boolean) {
        ...
        return Promise.all(
            targets.map((target) => checkImageLoaded(target))
        ).then(() => {
    ...
    public moves(movedInfos: MovedInfo[], isRestore?: boolean) {
        const frameMap = this.removeFrames(
